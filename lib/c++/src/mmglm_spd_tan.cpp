...
int main(int argc, char** argv)
{
  ...
  printf("################################################\n");
  printf("###               MMGLM_SPD_TAN              ###\n");
  printf("################################################\n");
  string myname = argv[0];
  if(argc == 1 || argc >= 6){
      cout<<"\n MMGLM_SPD_TAN performs MMGLMs in the tangent space of SPDs." <<endl;
      ...
      cout<<" Usage: " <<endl;
      cout<<"  >>> "<< myname <<" $input_dir $shared_dir"
          " ($result_fname==p_value.txt)" << endl;
      cout<<"  >>> "<< myname <<" $input_dir $shared_dir $result_fname (.05)" << endl;
      cout<<"  >>> "<< myname <<" $input_dir $shared_dir $result_fname $pval_threshold" << endl;
      cout<<"  >>> "<< myname <<" $input_dir $shared_dir $result_fname (negative number means no realy stop) " << endl;
      cout<<"\n ### Requirements ### "<<endl;
  ...
  cout<< "P value threshold is " << pvalth <<endl;
  ...
  // Formula to determine whether this voxel is promising or not
  // P value early stop logic
  // if counts/nperms > pval_th : stop
  // return counts/ncur_perms 
...
#endif
    //generalize this one
    p_value[ivoxel]=mmglm_spd_perm_tan(X, Y, idx_dti, pvalth);
  }
