...
arma::vec mmglm_spd_perm(const mat& X, const cube& Y, const imat & idx_dti){
    ...
    arma::vec ErrMx(1,nperms);
	// Check distance
    unsigned int idata;
    for(idata =0; idata <ndata;idata++){
        // accumulated errors in ErrMx
    	dist_M_pt2array(ErrMx,p, sqrtp, g, invg, Y, logYvhat_perm, idata);
    }
    return ErrMx;
}
